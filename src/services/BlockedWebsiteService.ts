...
export class BlockedWebsiteService {
  ...
  public async updateBlockedWebsite(id: string, name: string, url: string): Promise<void> {
    try {
      const oldWebsite = await (await dbObj)
        .select()
  ...
  public async deleteBlockedWebsite(id: string): Promise<void> {
    try {
      const website = await (await dbObj)
        .select()
        .from(blockedWebsites)
        .where(eq(blockedWebsites.id, id))
        .then(res => res[0]);
  ...
  private async addHostEntry(name: string, url: string): Promise<void> {
    try {
      ...
      sudo.exec(`echo ${JSON.stringify(entry)} >> "${this.hostsFilePath}"`, options, (error: any, stdout, stderr) => {
        if (error) {
          console.error(`Error adding host entry using echo command: ${stderr || error.message}`);
      ...
      console.error(`Failed to create host entry. Error: ${(error as any).message}`);
  ...
  private async addHostEntryFallback(entry: string): Promise<void> {
    try {
      console.log(`Fallback: Attempting to write entry directly to hosts file.`);
      fs.appendFileSync(this.hostsFilePath, entry, 'utf8');
      console.log('Fallback: Host entry added successfully.');
    } catch (error) {
      console.error(`Fallback: Failed to add host entry. Error: ${(error as any).message}`);
      throw new Error(`Fallback: Failed to create host entry. ${(error as any).message}`);
    }
  }
  ...
  private async removeHostEntry(url: string): Promise<void> {
    try {
        ...
        sudo.exec(command, options, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error removing host entry using PowerShell command: ${stderr || error.message}`);
                throw new Error(`Error removing host entry: ${stderr || error.message}`);
            }
